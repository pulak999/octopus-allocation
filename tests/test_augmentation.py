...
# ---------------------------------------------------------------------------
# 8e + 8f. End-to-end tests (require trace loading)
# ---------------------------------------------------------------------------
@pytest.mark.slow
def test_augmented_episode_runs():
    ...
    all_vms, node_to_vms, node_to_machine, vm_type_sz, machine_sz = trace_data
    M, _, _ = load_topology(
    ...
    env = OctopusMemPoolEnv(
        all_vms, node_to_vms, node_to_machine, machine_sz, M,
        seed=42, aug_config=config,
    )
    obs, info = env.reset(seed=42)
    assert "aug_params" in info
    assert info["num_events"] >= config.min_events
...
def test_no_augmentation_matches_baseline():
    ...
    env1 = OctopusMemPoolEnv(
        all_vms, node_to_vms, node_to_machine, machine_sz, M,
        seed=42, aug_config=None,
    )
    env2 = OctopusMemPoolEnv(
        all_vms, node_to_vms, node_to_machine, machine_sz, M,
        seed=42, aug_config=AugmentationConfig(enabled=False),
    )
...
# ---------------------------------------------------------------------------
@pytest.mark.slow
def test_skip_hotfix_includes_more_events():
    """With skip_hotfix=True, env should include >= as many events as default."""
    from octopus.data import load_trace, load_topology
    from octopus.env import OctopusMemPoolEnv
    ...
    env_hotfix = OctopusMemPoolEnv(
        all_vms, node_to_vms, node_to_machine, machine_sz, M,
        seed=42, skip_hotfix=False,
    )
    env_no_hotfix = OctopusMemPoolEnv(
        all_vms, node_to_vms, node_to_machine, machine_sz, M,
        seed=42, skip_hotfix=True,
    )

    _, info_hf = env_hotfix.reset(seed=42)
    _, info_no = env_no_hotfix.reset(seed=42)
    assert info_no["num_events"] >= info_hf["num_events"]
