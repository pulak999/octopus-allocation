...
In the current Octopus topology, each host is connected to exactly 8 MPDs.
That fixed degree keeps cabling and port count manageable while still giving
each server enough reach into the pool: a host can spread a VM's allocation
across up to 8 devices, and demand from different hosts is mixed on shared MPDs,
enabling statistical multiplexing.  The sparsity (8 links per host rather than
a link to every MPD) is what makes the allocation problem non-trivial---the
graph's \emph{expansion} (defined below) limits how well local demand spikes
can be absorbed by the rest of the pool.
...
\begin{itemize}
    \item \textbf{Pensieve} \cite{pensieve2017} (SIGCOMM '17): RL for adaptive bitrate streaming.
    \item \textbf{Decima} \cite{decima2019} (SIGCOMM '19): RL for scheduling in Spark workloads.
    \item \textbf{FleetIO} \cite{fleetio2025} (ASPLOS '25): RL for SSD capacity allocation across
...
Additionally, \textbf{Coach} \cite{coach2025} (ASPLOS '25) demonstrates that VM memory
...
Before committing to an RL approach we evaluate three analytical baselines---a
practical heuristic (Greedy), an exact offline optimum (Optimal), and a classical feedback controller (PID)---to bound the space of
...
terminates. Algorithm~\ref{alg:greedy} gives the per-VM allocation step.
...
\label{alg:greedy}
\KwIn{Request $x$ GB; accessible MPD set $\mathcal{N}$; current load vector $\mathbf{c} \in \mathbb{R}^m$}
...
\While{$x > 0$}{
    ...
    \tcp{next-level load, or $\infty$ if none}
    $\delta \leftarrow \min\bigl(x,\; |S| \cdot (c_{j'} - c_{j^*})\bigr)$\;
    \ForEach{$j \in S$}{
        $a_j \leftarrow a_j + \delta / |S|$; \quad $c_j \leftarrow c_j + \delta / |S|$\;
    }
    $x \leftarrow x - \delta$\;
}
\Return $\mathbf{a}$\;
...
$O(T \cdot n \cdot m \cdot (n + m)^{0.5})$ time; Algorithm~\ref{alg:optimal}
gives the high-level procedure.

\begin{algorithm}[H]
\SetAlgoLined
\DontPrintSemicolon
\caption{Optimal allocation via binary search on peak load.}
\label{alg:optimal}
\KwIn{Per-server demands $\mathbf{b} \in \mathbb{R}^n$; adjacency matrix $M \in \{0,1\}^{n \times m}$}
\KwOut{Minimum peak per-MPD load $t^*$ achievable by a feasible allocation}
\BlankLine
$t_{\mathrm{lo}} \leftarrow \text{lower\_bound}(\mathbf{b}, M)$\;
\tcp{e.g.\ max of row-sums divided by degree}
$t_{\mathrm{hi}} \leftarrow \sum_{i=1}^{n} b_i$\;
\tcp{trivial upper bound}
\For{$k \leftarrow 1$ \KwTo $30$}{
    \tcp{$\approx 10^{-9}$ relative precision}
    $t \leftarrow (t_{\mathrm{lo}} + t_{\mathrm{hi}})/2$\;
    \eIf{$\text{feasible}(t; \mathbf{b}, M)$}{
        $t_{\mathrm{hi}} \leftarrow t$\;
    }{
        $t_{\mathrm{lo}} \leftarrow t$\;
    }
}
\Return $t_{\mathrm{hi}}$\;
\tcp{optimal peak per-MPD}
\end{algorithm}
...
\subsubsection*{Initial State Space (v0)}
...
\subsubsection*{New State Space (v1)}
...
\begin{tabular}{clp{3.8cm}p{3.2cm}lp{4.2cm}}
\toprule
\textbf{\#} & \textbf{Category} & \textbf{Signal} & \textbf{Temporal Treatment} & \textbf{Name} & \textbf{Encoding} \\
\midrule
1 & Res.\ Health & Per-MPD capacity util.\ (\% full)
  & 3 windows, 1-hr & \texttt{mpd\_cap}
  & $[v_t,\Delta_{1h},\Delta_{2h}]\times N_\text{MPD}$ \\
2 & Res.\ Health & Per-MPD bandwidth util.\ (\% full)
  & Current snapshot & \texttt{mpd\_bw}
  & $[v_t]\times N_\text{MPD}$ \\
3 & Res.\ Health & Server memory pressure (\% RAM)
  & 3 windows, 1-hr & \texttt{mem\_press}
  & $[v_t,\Delta_{1h},\Delta_{2h}]\times N_\text{srv}$ \\
4 & Res.\ Health & Recent SLO violations
  & 24-hr summary & \texttt{slo\_vio}
  & Scalar (\% allocs causing overflow) \\
5 & Workload & Incoming VM size
  & Current snapshot & \texttt{vm\_size}
  & Scalar (GB) \\
6 & Workload & Predicted VM lifetime
  & Current snapshot & \texttt{vm\_lifetime}
  & Scalar (hours) \\
7 & Workload & Peak-to-average ratio
  & 24-hr summary & \texttt{pta}
  & $\max/\operatorname{mean}(\text{usage}_{24h})$ \\
8 & Workload & Predicted memory growth rate
  & Current snapshot & \texttt{future\_mem}
  & Scalar (GB/hr) \\
9 & Topology & Server-to-MPD connectivity mask
  & Static & \texttt{conn\_mask}
  & Binary $\in \{0,1\}^{N_\text{MPD}}$ \\
\bottomrule
\end{tabular}
\caption{v1 state space. $v_t$: current value; $\Delta_{1h}$, $\Delta_{2h}$:
deltas from 1 and 2 hours prior. \texttt{conn\_mask} doubles as the action
validity mask.}
\label{tab:states}
\end{table}
...
remaining weights to satisfy Equation~\ref{eq:sum}.

Two action types are exposed to the RL agent: (1)~the continuous allocation
split $\mathbf{a}$ across connected MPDs (Equation~\ref{eq:action}), and
(2)~a binary \texttt{flag\_migration(vm\_id)} flag that marks a VM for
migration. Physical page placement and address mapping are handled
mechanically by the system and are transparent to the agent.
...
\subsubsection*{Episode Structure and Replay}

Training uses the Octopus simulator as the RL environment via
Stable-Baselines3~\cite{stable_baselines3}.  Each \emph{episode} replays
one full production VM trace (two weeks at 5-minute resolution).  At every
VM arrival event the agent receives an observation, outputs an allocation
action, and receives the immediate reward.  Episode termination occurs when
the trace is exhausted; the environment resets by reshuffling the pod-to-node
assignment (random seed incremented) and replaying the same trace, providing
variation in which servers land in the same pod.

SAC is off-policy: all transitions $(s_t, a_t, r_t, s_{t+1})$ are stored in
a replay buffer and sampled uniformly for gradient updates.  A single
training environment suffices; a separate deterministic evaluation
environment (a different trace, \texttt{LON23}) runs every
\texttt{eval\_freq} steps to track generalisation.

\subsubsection*{Hyperparameter Comparison: Slow vs.\ Fast Runs}

Two training configurations were used, both training SAC for 1\,M timesteps
on the \texttt{AMS20} trace with the \texttt{AG16x6\_expander\_quads}
topology (96 hosts, 192 MPDs, $d_{\max}=8$):
...
\begin{tabular}{lcc}
\toprule
\textbf{Hyperparameter} & \textbf{Slow (6.5 hr)} & \textbf{Fast (34 min)} \\
\midrule
Network architecture & $[256, 256]$ & $[128, 128]$ \\
Replay buffer size   & 1\,000\,000  & 300\,000 \\
Batch size           & 256          & 512 \\
\texttt{train\_freq} & 1            & 16 \\
Checkpoint interval  & 10\,000 steps & 50\,000 steps \\
Eval interval        & 20\,000 steps & 100\,000 steps \\
Gradient updates at 1M steps & $\sim$999\,000 & $\sim$62\,000 \\
Final eval reward    & $-3.17 \pm 0.87$ & $-2.71 \pm 0.00$ \\
Device               & \texttt{cuda:0}  & \texttt{cuda:1} \\
\bottomrule
\end{tabular}
\caption{Comparison of slow and fast SAC training configurations.  Both
train for 1\,M environment steps on AMS20, evaluated on LON23.  The fast
run uses \texttt{train\_freq=16}: one gradient step per 16 environment
steps, yielding $\sim$16$\times$ fewer updates.  The fast run achieves a
slightly better final eval reward, suggesting the slow run may be
overfitting.}
\label{tab:training_configs}
\end{table}

The key difference is \texttt{train\_freq}: the slow run performs a gradient
update at \emph{every} environment step ($\sim$999\,k updates total), while
the fast run updates every 16 steps ($\sim$62\,k updates total).  Despite
far fewer gradient steps, the fast run converges to a marginally better
evaluation reward.  This is consistent with the known SAC behaviour that
excessive updates on correlated sequential data can lead to overfitting
on the replay buffer.

\subsubsection*{Deployment Considerations}

In production, the RL agent would run as a centralised controller on the
pod management plane.  At each VM arrival (an event-driven trigger), the
controller constructs the observation vector, runs a forward pass through
the policy network, and returns the allocation split---all within the VM
scheduling latency budget ($\ll 1$\,ms for a two-layer MLP).  No gradient
updates occur at inference time; the model is frozen after offline training
and re-trained periodically (e.g.\ weekly) on fresh trace data.

The \texttt{best\_model} checkpoint (saved by \texttt{EvalCallback} whenever
the LON23 evaluation reward improves) is the natural candidate for
deployment, as it represents the policy that generalised best to the
held-out cluster during training.
...
\subsection{v0 Simulation Results}
...
% ----------------------------------------------------------------------
\subsection{Slow vs.\ Fast Model Comparison}
\label{sec:slow_vs_fast}
% ----------------------------------------------------------------------

\red{TODO (experiments needed --- see Section~\ref{sec:next_steps}).}

Both trained checkpoints exist (\texttt{output/checkpoints/best\_model.zip}
for the slow run; \texttt{output/checkpoints\_fast/best\_model.zip} for the
fast run).  The experiments required to populate this section are:

\begin{enumerate}
    \item Run \texttt{evaluate.py --policy greedy rl --model output/checkpoints/best\_model} and
          \texttt{output/checkpoints\_fast/best\_model} on both AMS20 and
          LON23 traces, 50 pod mappings each.  Record pooling savings
          (avg, min, max, std).
    \item Plot per-MHD load time-series for both models side-by-side
          (analogous to Figures~\ref{fig:ts_greedy}/\ref{fig:ts_rl}) to
          visualise whether the fast model learns better load balance.
    \item Plot training curves (eval reward vs.\ timestep) from
          \texttt{output/logs/evaluations.npz} and
          \texttt{output/logs\_fast/evaluations.npz} on the same axes.
\end{enumerate}

The expected finding is that the fast model's coarser update schedule
reduces overfitting on the training trace and generalises better to LON23,
consistent with the final eval rewards in Table~\ref{tab:training_configs}.

% ----------------------------------------------------------------------
\subsection{Scaling to 96-Host Pods}
\label{sec:96hosts}
% ----------------------------------------------------------------------

\red{TODO (experiments needed --- see Section~\ref{sec:next_steps}).}

All RL results to date use the \texttt{AG16x6\_expander\_quads} topology
(16 hosts, 6 MPDs per pod).  The production Acadia target is a 96-host pod
with 120 MPDs (80 internal + 40 external).  The experiments required to
populate this section are:

\begin{enumerate}
    \item \textbf{Greedy and optimal baselines at 96 hosts.}
          Run \texttt{evaluate.py --policy greedy --topology
          data/topologies/<96h\_topology>.csv} on all ten Azure traces, 50
          pod mappings each.  This establishes the savings budget the RL
          policy must beat.
    \item \textbf{RL training at 96 hosts.}
          Retrain SAC with the fast configuration on the 96-host topology.
          The observation vector grows from 20 to $2 \times 8 + 4 = 20$
          dimensions (same $d_{\max}=8$), so no architectural change is
          needed; the pod-size increase affects only simulation runtime.
    \item \textbf{Evaluate generalisation across topologies.}
          Test the 96-host model on the Acadia-96 topology ($N=4$,
          $m=120$) and compare pooling savings against greedy and the
          static-optimal bound from Table~\ref{tab:greedy_vs_opt}.
\end{enumerate}
